...
TEST_F(SDPChordalTest, Feasible)
{
    ...
    DefaultSettings<double> settings = DefaultSettingsBuilder<double>::default_settings()
        ...
        .chordal_decomposition_merge_method(ClarabelChordalDecompositions::CliqueGraph)
        .max_iter(50)
        .build();

    Vector<bool, 2> compact_vec = {false, true};
    Vector<bool, 2> complete_dual_vec = {false, true};
    Vector<ClarabelChordalDecompositions, 3> merge_method_vec = {ClarabelChordalDecompositions::CliqueGraph,
                                                                 ClarabelChordalDecompositions::ParentChild,
                                                                 ClarabelChordalDecompositions::None};


    for (auto &compact : compact_vec){
        for (auto &complete_dual : complete_dual_vec){
            for (auto &merge_method : merge_method_vec){
